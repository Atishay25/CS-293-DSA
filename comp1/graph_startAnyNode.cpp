...
void Graph::modifiedDFS() {
  ...
  for(int j = 0; j < numNodes; j++){
    if(nodes[i%numNodes]->visited == 0){
      TreeNode *root = new TreeNode(nodes[i%numNodes]->value);
      BST *tree = new BST();
      dfs(nodes[i%numNodes],tree);
      tree->printBST("",false);
      connectedComponents++;
    }
    i++;
  ...
  for(int i = 0; i < numNodes; i++){
    if(nodes[i]->parent == nullptr) cout << nodes[i]->value << "   , parent : " << "NULL" << endl;
    else cout << nodes[i]->value << "   , parent : " << nodes[i]->parent->value << endl;
  }
  // dfs(nodes[0]);
  return;
}
