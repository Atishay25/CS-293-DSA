    ...
    while(!stnQueue.isEmpty()){           // loop through stations until stnQueue is empty
      ...
      while(stnAdj != nullptr){           // Loop through each station adjacent to the current station (stn)
        ...
        if(stnAdj->object->adjacentStnIndex == stnNameToIndex.get(destStnName)->value){   // If stnAdj is the destination
          ...
          listOfObjects<StationConnectionInfo *> *srcTrains_temp = srcTrains;
          while(srcTrains_temp != nullptr){                    // Check if train with current JourneyCode is departing from adjacent station (stnAdj)
          newDirectTrain = find(itrJourneyCode->object,srcTrains_temp->object->trains);
          cout << "IN\n";
          if(newDirectTrain != nullptr){
            break;
          }
          srcTrains_temp = srcTrains_temp->next;
        }
